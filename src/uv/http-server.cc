...
namespace http {
  ...
  int Server::complete (http_parser* parser, Listener cb) {
    ...
    setRequests(req, uri::ExtractSearch(context->url));
    setRequests(req, context->body.str());
  ...
  void Server::setRequests (Request &req, string body) {
    if (body.empty()) {
        return;
    }

    auto pairs = uri::Split(body, "&");

    for(auto &p : pairs) {
      auto pair = uri::Split(p, "=");
      if (pair.size() == 2) {
        req.requests.insert({ pair[0], uri::UriDecode(pair[1]) });
      }
    }
  }
